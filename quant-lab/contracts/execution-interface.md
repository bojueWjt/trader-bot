...
### 5.21 裁定 B19：强平记账归属 + 更正 B18 §4 + 跨窗口单一来源（G0 OR-02 R47，规范性）

G2 指出 B18 定主口径为 B 后的架构后果：**强平记账若在 G2 与 G3 各写一份，就是把族 A（一式多处实现）制度化到跨窗口**，而 §12 A24 的树级 lint 与调用点登记**只扫单个窗口的 `src/`，跨窗口的第二份实现对它们是隐形的**。该判断成立，且比窗口内的同类问题更难发现。

#### 1. 采纳选项一，但决定性理由不是成本，而是"强平不是事件，是估值"

**裁定：`force_close_net_R(...)` 由 G2 在契约层出唯一实现，G3 只调用不自算；C06 落地时内核的政策平仓腿调同一函数。** 不采纳选项二（把 C06 提前进 P1）。

**理由（比单一来源更根本）**：B 口径的"观察终点强制平仓"**是一个研究估值约定，不是一件发生过的事**——仓位在世界上从未平掉，我们只是按 `horizon_end` 的 mark 给余仓记了个价。把它实现成内核的平仓腿（选项二），等于**把一个记账约定伪装成一个发生过的动作**，这正是 §9.10.11 A15 与 §5.12 B10 反复在堵的形态。

**由此附三条硬约束**：
1. `force_close_net_R` **不得产生 `canonical_events`**，不得改变 `fill_status`、`outcome_kind` 或 `censor_reason`——执行记录必须继续如实说"我们没看到它平仓"；
2. 强平值是**研究层派生量**，须带独立字段名（如 `net_R_forced`）与出处标记（`estimand=forced_close`、所用 mark 的时刻与来源），**不得覆写 `net_R`**；
3. 单一实现必须同时处理 G2 列出的五项：已实现部分（多档 TP 可能已部分平仓）、余仓方向与数量、`horizon_end` 处**最后一根已闭合** mark（**不得取未来价**）、余仓平仓的 taker 费、累计费与资金费。**任一项在第二份实现里写错都会安静地改变 θ，因为两边各自自洽、没有断言会响。**

#### 2. 更正 G0 在 §5.20 B18 第 4 节的裁定

B18 §4 称"采纳 B 后 `right_censored` 的观察窗路径不可达"。**该结论建立在"B 由内核实现"这一未言明的前提上，按本节第 1 条该前提不成立，故更正**：

**执行记录中 `right_censored` 的两条路径（`kernel_a.py:618` 的 `hold_end` 与 `:621/:644` 的 `horizon`）在 B 口径下仍然全部可达**——因为强平只发生在研究层。B 改变的不是内核是否判删失，而是**研究层不再丢弃这些 episode**。

**保留 B18 §4 的另一半**：两条路径仍须**分别断言**、各自注明可达性与理由，不得合并。G2 自查已发现其实验全部 `max_holding=None`、第二条一次未触发，故"需强平样本 100% 可强平"只覆盖观察窗路径——该自查正确，测试须按两条写。

#### 3. 跨窗口单一来源：A24 机制的盲区补丁

**裁定**：凡一个公式需跨窗口共享，**必须由属主窗口出函数、消费窗口 `import` 调用，禁止在消费窗口重新实现**；属主窗口的**调用点登记须列出跨窗口调用方**；消费窗口须有一条测试断言**该量来自 import 而非自算**（monkeypatch 属主函数为哨兵，断言消费方输出随之改变——§11 A23 的行为证明法）。

理由：A24 的两条机制按窗口扫树，跨窗口重复对它们隐形。**把共享点收敛成一次 import，就把跨窗口问题转化成属主窗口内的调用点问题**，两条机制随即重新生效。

#### 4. 最小有效簇数：数值与口径必须一起定（准 G2 所请）

G2 指出：连通分量计数是三种有效样本口径里**最严**的一种，**B 在任何长于信号间隔的观察窗上都会被它自动判 0**；用平均唯一度加权或块自举则未必。

**裁定**：§5.20 B18 第 5 节所要求的"折内最小有效簇数下限"，**其数值与计数口径必须同时确立，单定数值没有意义**。G2 + G3 提案时须给出**至少两种口径**（连通分量、平均唯一度加权 / 块自举）下的对照表，并说明所选口径为何与 `max-t` 的重采样单位一致——**下限与重采样单位不一致时，这个门就管不住它想管的东西**。

#### 5. 记录 G2 关于"归一"的观察

同一个词、两种后果：`vision.expected_rows` 归一在对齐边界上两公式恒等，属**纯增益**（已要求等价性测试）；funding 归一会把 **16% 真实结算行判脏**，属**事故**。**这正是 §5.20 B18 第 2 节补的对称半条所管的冲动**——例外要有证据，统一也要有证据。

### 5.22 裁定 B20：撤回"B 以有效簇数为代价"的前提；最小有效样本下限按重采样单位定（G0 OR-02 R50，规范性）

#### 1. G0 独立核实 G2 的撤回：前提确实错了

§5.20 B18 第 5 节与 §5.21 B19 第 4 节据以立论的"删失率与有效簇数**反向**"，其证据是**连通分量**口径。G2 读码后撤回该结论，G0 **直接读源核实属实**（`src/quant_lab/research/maxt.py:85 calendar_blocks`）：

- 块由**日历整除**切分：`raw = int((t - origin) / L)`，**不因样本重叠而合并**；
- 整簇归**最早 `t_dec`** 所在块：`first[c] = min(...)`；
- 准入门数 `n_nonempty` 只数**共同支持 `W > 0`** 的块（`W = mask × weights`），全删失块保留在重采样矩阵里但不计入；
- `cross`（跨块簇占比）与 `max_span` 是已有的跨块诊断量。

**即：max-t 的重采样单位是日历块，而连通分量会因重叠合并——两者根本不是同一个量。** 同一批数据三口径对照（G2 提供，口径定义与上述源码一致）：

| 观察窗 | 连通分量 A / B | 平均唯一度 A / B | **非空日历块 A / B（= max-t 单位）** |
|---|---|---|---|
| 1d | 22 / 24 | 30.2 / 36.5 | **22 / 24** |
| 3d | 6 / **1** | 28.4 / 22.4 | **8 / 8** |
| 5d | 3 / **1** | 22.3 / 19.4 | **5 / 5** |
| 7d | 2 / **1** | 20.5 / 18.3 | **4 / 4** |

**按实际重采样单位，B 在每个观察窗上都不劣于 A（1d 更优，其余持平）。只有连通分量这一种口径会让 B 看起来灾难性地差，而它恰恰不是 max-t 用的那个单位。**

**裁定**：B18 §5 / B19 §4 中"删失率与有效簇数反向、B 以有效样本为代价"的表述**作废**。**口径 B 的无偏是净得，不以有效样本数为代价。**

#### 2. 新记一种失效形态：用错单位的度量不只是量不准，它会捏造一个取舍

§5.21 B19 第 4 节曾写"下限与重采样单位不一致时，这个门就管不住它想管的东西"。**该表述不够**。G2 的经历给出更严重的后果，予以采纳并记为一般规则：

> **下限与重采样单位不一致时，不只是门管不住想管的东西，它还会把一个不存在的取舍写进决策。**

这是继"永不触发"（§10 A22 / §12 A24）与"因噪声触发"（§9.10.17 A27）之后的**第三种形态**：门**本身工作正常**，但它度量的是**另一个量**，于是产出一个**看起来可信、实则虚构的权衡**，并据此改变裁定。**前两种让门失效，这一种让决策失真**——G0 的 B18 正是被它改变过的裁定之一。

**通则**：凡用于准入、下限或取舍论证的统计量，**必须与它所服务的推断过程使用同一个单位**，且该一致性须在提案中**逐字对照实现**（如"口径 = `maxt.calendar_blocks` 的 `n_nonempty`"），不得只写口径名称。

#### 3. 最小有效样本下限：采纳 G2 的四条

1. **口径取非空日历块 `n_nonempty`**，与 `maxt.calendar_blocks` 逐字一致（§2 的一致性要求）。
2. **下限与 `block_len` 必须成对声明**，且 `block_len ≥ research_horizon_s + purge/embargo`。**只写块数不写块长，同一个数字可以对应任意强度。**
3. **同时钉住 `cross` 与 `max_span`**（`BlockIndex` 已有）：为 0 与非 0 含义完全不同，而只看块数看不出来。
4. **数据量要求由切分算术直接给出**：`block_len ≥ 观察窗` 是块自举独立性的前提，故块数上限 = 日历长度 ÷ 观察窗。**5d 窗口下达到 Q 个非空块，每折约需 Q×5 天日历**（Q=20 → 100 天，Q=30 → 150 天）。

#### 4. 关键区分：这是数据量结论，不是取值结论

2024-01 单月在 5d 下只有 **5 块**，**任何合理下限都过不了**。但这**不否定 5d 这个取值**，它否定的是"**单月数据能支撑 5d 观察窗的 θ 声明**"。

**裁定**：`research_horizon_s = 5d` **解除占位**（B18 第 3 节所附"待最小簇数约束确立"的条件，其依据已被 §1 撤销）；同时**新增一条 P2 前置**：任何基于 5d 观察窗的 θ 声明，其每折日历长度须满足 §3 第 4 条的数据量要求，否则只能作描述性报告。该前置属 `G-STAT-CLAIM` 实质内容。

**这条区分本身值得记**：一个门没过，可能是取值错了，也可能是数据不够——**把后者误报成前者，会让人去调一个本来正确的参数**。

### 5.23 裁定 B21：信心状态不进内容哈希；可追溯性挂在声明上，不挂在哈希上（G0 OR-02 R51，规范性）

#### 1. 确认 G2 的读法：解除占位不触发 B4 bump（G0 读源核实）

| 待核事实 | G0 核实 |
|---|---|
| `research_horizon_s` 取值自 B16 起未变 | `resolve_policy("base-v1").research_horizon_s = 432000` = 5.0 天 ✓ |
| 占位标注不参与版本查找 | `contract.py:371 load_policy_registry()` 逐字为"下划线开头的键是元数据（占位状态等），不参与版本查找"，实现为 `if not k.startswith("_")` ✓ |
| `content_hash` 不读该 JSON 元数据 | `contract.py:338` `content_hash = sha256_canonical(self.model_dump(mode="json"))`，来源是模型不是文件 ✓ |

**变的只是决策状态（暂定 → 定稿），内容未变。§5.5 B4 要求 bump 的条件未触发，不 bump。** 原 `_note` 中"最终值定稿时按 B4 bump 到 v2"写于"最终值可能不同于 5d"的假设下，该假设已被 §5.22 B20 取消，**该句作废**。

#### 2. 裁定：信心状态**不得**进内容哈希

采纳 G2 的论证并立为规则：**内容哈希标识"这个数是什么"，不标识"我们对它有多确定"。**

把信心编码进内容哈希有两个具体后果：
1. **同一份经济事实会因我们的心态不同而产生两个身份**——两个 `policy_hash` 对应完全相同的经济内容，此后任何按哈希做的等同性判断都会误判；
2. **诱使为留痕而制造一次没有内容变化的 bump**，从而把 B4 的语义（"数值变了就换身份"）稀释成"状态变了也换身份"，B4 随即不再能回答它唯一该回答的问题。

#### 3. 但 G2 指出的可追溯性缺口是真的，按其实际影响处置

G2 主动写明：内容未变则**占位期产出的结果与定稿后产出的结果在 `trace_hash` 上完全不可区分**，并把这条写进 `report-G2-research-horizon-proposal.md` §7.2 而非只写"不需要 bump"。**该做法正确**——§10 A22 已立"在覆盖一份记录之前先让它可恢复"，此处同理：**一条真实的缺口不应该靠不提它来处理**。

**但该缺口的影响面比它看起来窄**：占位与定稿期间**经济内容完全相同**，因此**结果本身没有差别**；有差别的是**我们当时是否愿意据此发表声明**。也就是说，这是**关于声明的可追溯性，不是关于结果的**。

**裁定**：可追溯性挂在**声明**上，不挂在哈希上——**任何 θ 声明必须记录其作出时所依据的契约版本（`contracts.version` 与 changeLog 编号）**。如此"该声明是在占位期还是定稿后作出的"可被精确回答，而内容寻址保持纯粹。该字段属 `G-STAT-CLAIM` 的必填项。

**一般原则**：**当一个身份系统回答不了某个问题时，先确认那个问题是不是该由它回答。** 把所有追溯需求都塞进哈希，会让哈希既不能准确回答新问题、也损失了它原本能准确回答的那个。
