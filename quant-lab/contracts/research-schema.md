...
##### 9.10.20 第 4 款：事故 / 对抗的**操作判据**（G0 R-10 裁定补，2026-09-12）

§9.10.20 同时写了两件事：第 1 款说**事故不免责**，末段说**验收边界本来就不含对抗**（"不要求防御任意改写全部执行与验证代码的恶意进程；目标是消除正常导入与缓存混版"）。G3 在 R-10 第 15 轮读到第 1 款即停，结论却在末段。**这是 G0 的写法造成的**：禁令写得斩钉截铁、范围限定另起一段放在最后。补一条**可机械执行**的归属判据：

> **能不能在「不向 worker 进程内注入代码」的前提下复现？**

| 答 | 类别 | 处置 |
|---|---|---|
| **能**——来源是文件系统状态（陈旧 `__pycache__`）、进程继承（fork 带入父模块对象）、普通导入顺序 | **事故类** | **必须挡住**，P1 必修 |
| **不能**——复现必须在 worker 进程内执行代码去绑定 globals / 改注册表项 / 改类属性 / 换默认参数 | **对抗类** | **A39 适用**，不属 P1 验收边界 |

**并立一条方法边界**：对「任意外部 Python callable」这个支持域，**自动推导其完整行为依赖、并判断状态差异是否影响未来行为，不可判定**。故**事故类必须以结构性隔离关闭，不得以运行状态反射关闭**——反射路线每修一条必有同族下一条，这是机械后果，不是实现水平问题。
