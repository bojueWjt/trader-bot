...
class BinanceCommonExecutionClient(LiveExecutionClient):
    ...
    async def generate_order_status_reports(
        ...
        try:
            ...
            if command.open_only:
                ...
                # beyond its ~7-day retention window even when they are STILL
                # OPEN, so a resting order older than that vanished from
                # reconciliation. The openOrders snapshot is authoritative for
                # live orders — merge it in (dedupe on symbol+orderId).
                _seen_ids = {(o.symbol, o.orderId) for o in binance_orders}
                for _open_order in binance_open_orders:
                    if (_open_order.symbol, _open_order.orderId) not in _seen_ids:
                        binance_orders.append(_open_order)
        except BinanceError as e:
    ...
    def _parse_order_status_reports(
        ...
        for order in binance_orders:
            # PATCH 2026-07-10 (hk): never time-filter orders that are still
            # OPEN on the venue. The reconciliation lookback window is meant to
            # bound closed-order history, but a resting limit order older than
            # the window was silently dropped here, so every node restart
            # forgot all pre-restart resting orders (cancel intents then died
            # with denied:order_not_found while the orders stayed live).
            _status_raw = getattr(order, "status", None)
            _is_open = str(getattr(_status_raw, "value", _status_raw) or "").upper() in (
                "NEW",
                "PARTIALLY_FILLED",
            )
            if not _is_open:
