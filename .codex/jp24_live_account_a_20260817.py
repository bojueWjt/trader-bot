...
RELEASE_ROOT = Path("/srv/trader-staging/trader-jp24-release-dbdf167-20260818T071337Z")
RELEASE_ID = "37b5b1e5104b5594e82ed7127dfafc8ae29450d0a39223b36cf4c455a60f02a7"
IMAGE_DIGEST = "sha256:f65bc2f2ddaa056480aa770695aa4b8779ddbd189b496485398662a8172c0973"
REVIEWER_PRIVATE_KEY = Path("/srv/trader-v3/account-a-canary/reviewer/reviewer-20260814T022842Z/reviewer-private.pem")
REVIEWER_TRUST_SHA256 = "82063488c91fdd51edb4dc9fcddc918f3ef1d350df6b5ea23f07eaa740761f81"
...
def load_heartbeat(conn, target: Target) -> dict[str, Any]:
    with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
        cur.execute("""
    ...
    if age < -MAX_HEARTBEAT_FUTURE_SKEW_SECONDS or age > 5:
...
def wait_for_fresh_evidence(conn, target: Target) -> dict[str, Any]:
    ...
    while time.monotonic() < deadline:
        row = load_heartbeat(conn, target)
        last_ages = {
...
def get_public_price() -> tuple[Decimal, dict[str, Any]]:
    with urllib.request.urlopen(f"https://fapi.binance.com/fapi/v1/ticker/price?symbol={SYMBOL}", timeout=8) as response:
        ticker = json.loads(response.read().decode("utf-8"))
    with urllib.request.urlopen(f"https://fapi.binance.com/fapi/v1/exchangeInfo?symbol={SYMBOL}", timeout=8) as response:
...
def run_account(base_dir: Path, target: Target, expected_phase: str, next_phase: str | None) -> None:
    ...
    try:
        ...
        request.update({
            "phase": "preflight",
            "side_effect_id": (
                f"{target.account_id}-pre-permit-refresh-{permit_id}"
            ),
        })
        refreshed = adapter_call("refresh-evidence", request, env)
        log("PRE_PERMIT_REFRESH " + json.dumps({
            "account_id": target.account_id,
            "accepted": refreshed.get("accepted"),
            "command_id": refreshed.get("command_id"),
            "command_status": refreshed.get("command_status"),
        }, sort_keys=True))
        wait_for_fresh_evidence(conn, target)
    ...
    if next_phase is not None:
        advance_phase(account_dir, target, next_phase)

def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(
        description="Run or advance the jp-24 reviewed four-account rollout.",
    )
    result.add_argument(
        "--account",
        choices=("account-b", "account-c", "account-d"),
        help="execute one live canary and advance the rollout",
    )
    result.add_argument(
        "--advance-account-a-closure",
        type=Path,
        help="advance the signed account-a closure directory to account-b",
    )
    return result
...
def main(argv: list[str] | None = None) -> int:
    ...
    selected = int(bool(args.account)) + int(
        args.advance_account_a_closure is not None
    )
    if selected != 1:
        raise RuntimeError(
            "select exactly one of --account or --advance-account-a-closure"
        )
    if args.advance_account_a_closure is not None:
        target = TARGET_BY_ACCOUNT["account-a"]
        account_dir = args.advance_account_a_closure.resolve()
        advance_phase(
            account_dir,
            target,
            NEXT_PHASE_BY_ACCOUNT[target.account_id],
        )
        conn = db_connect()
        try:
            require_phase(conn, NEXT_PHASE_BY_ACCOUNT[target.account_id])
        finally:
            conn.close()
        log("ACCOUNT_A_CLOSURE_ADVANCED")
        return 0
    ...
    run_account(base_dir, target, target.phase, next_phase)
    conn = db_connect()
    try:
        require_phase(conn, next_phase)
    finally:
        conn.close()
