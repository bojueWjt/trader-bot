...
class TakeProfitLevel(BaseModel):
    ...
    def _price_and_size_are_xor(self) -> "TakeProfitLevel":
        if (self.price is None) == (self.r_multiple is None):
            raise ValueError(
                "take_profit level requires exactly one of price or r_multiple"
            )
        if (self.fraction is None) == (self.quantity is None):
            raise ValueError(
                "take_profit level requires exactly one of fraction or quantity"
            )
        return self
...
class Protection(BaseModel):
    ...
    def _take_profit_fraction_cap(self) -> "Protection":
        total = sum(
            (
                level.fraction
                for level in self.take_profits or []
                if level.fraction is not None
            ),
            Decimal("0"),
        )
...
class ApprovedTradeIntentV1(BaseModel):
    ...
    are optional additive v1 fields. Their absence preserves legacy intents.
    """
