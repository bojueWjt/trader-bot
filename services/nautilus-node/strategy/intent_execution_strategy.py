...
class _ProtectionStashDelta:
    intent_key: str
    entry: Mapping[str, Any] | bool = False
    deleted: bool = False
...
class _ProtectionStashTask:
    version: int
    deltas: tuple[_ProtectionStashDelta, ...]
...
@dataclass(frozen=True)
class _IntentReceiptTransitionTask:
    intent: Any
    intent_id: str
    expected_status: str
    status: str
...
class IntentExecutionStrategy(Strategy):
    ...
    def __init__(self, config: IntentExecutionStrategyConfig) -> None:
        ...
        self._pending_opening_reconciliations: dict[
            str,
            Any,
        ] = {}
        self._opening_reconciliation_resubmit_allowed: dict[
            str,
            bool,
        ] = {}
        ...
        self._opening_resubmit_transition_pending_ids: set[
            str
        ] = set()
        self._opening_reconciliation_denial_keys: dict[
            tuple[str, str, str],
            None,
        ] = {}
        self._opening_evidence_batch_cursor = 0
        ...
        self._durable_stash_canonical: dict[
            str,
            dict[str, Any],
        ] = {}
    ...
    def on_start(self) -> None:
        ...
        self._entry_protection_stash = (
            self._load_entry_protection_stash()
        )
    ...
    def _on_exchange_state_timer(self, *_args: Any, **_kwargs: Any) -> None:
        ...
        self._queue_exchange_refresh(
            ...
            success_continuation={
                "kind": "retry_exchange_state_dependents",
    ...
    def _refresh_mirror(
        ...
        try:
            refresh(
                client_order_ids=target_client_order_ids
            )
        except TypeError as exc:
            if not re.search(
                r"unexpected keyword argument|takes no keyword arguments",
                str(exc),
            ):
                raise
            refresh()
    ...
    def _queue_entry_protection_stash_persist(
        ...
        *,
        continuation: Mapping[str, Any] | bool = False,
        changed_intent_keys: str | Iterable[str] | bool = False,
    ) -> bool:
        ...
        if isinstance(continuation, Mapping):
            task_continuation = MappingProxyType(
                dict(continuation)
            )
        changed_keys = self._protection_changed_intent_keys(
            changed_intent_keys,
            task_continuation,
        )
        deltas = tuple(
            self._protection_stash_delta(intent_key)
            for intent_key in changed_keys
        )
        continuation_key = self._protection_continuation_key(
            task_continuation,
            version,
        )
        self._protection_continuation_versions[
            continuation_key
        ] = version
    ...
    def _process_durable_io_task(
        ...
        if isinstance(task, _IntentReceiptTransitionTask):
            handler = self._intent_receipt_transition_handler
            if handler is None:
                raise RuntimeError(
            ...
            return
        if not isinstance(task, _ProtectionStashTask):
            raise TypeError("unsupported strategy durable I/O task")
        if task.version < 1:
            raise ValueError(
        ...
        for delta in task.deltas:
            if delta.deleted:
                self._durable_stash_canonical.pop(
                    delta.intent_key,
                    None,
                )
                continue
            ...
            if not isinstance(entry, Mapping):
                raise ValueError(
                    "protection stash delta requires an entry"
                )
            self._durable_stash_canonical[
                delta.intent_key
            ] = self._jsonable_protection_stash_value(
                dict(entry)
            )
        self._write_entry_protection_stash(
            self._durable_stash_canonical
        ...
        self._queue_durable_io_result(
            _ProtectionStashResult(task=task)
        )

    def _queue_durable_io_result(self, result: Any) -> None:
        try:
    ...
    def _protection_changed_intent_keys(
        self,
        changed_intent_keys: str | Iterable[str] | bool,
        continuation: Mapping[str, Any] | bool,
    ) -> tuple[str, ...]:
        values: Iterable[Any] = ()
        if isinstance(changed_intent_keys, str):
            values = (changed_intent_keys,)
        elif changed_intent_keys is not False:
            values = changed_intent_keys
        keys = tuple(
            dict.fromkeys(
                str(value)
        ...
        if keys:
            return keys
        if isinstance(continuation, Mapping):
            explicit = continuation.get(
                "protection_changed_keys"
            )
            ...
            intent_id = str(
                getattr(plan, "intent_id", "") or ""
            )
            if intent_id:
                return (intent_id,)
        if len(self._entry_protection_stash) == 1:
            return (next(iter(self._entry_protection_stash)),)
        return ()

    def _protection_stash_delta(
        self,
        intent_key: str,
    ) -> _ProtectionStashDelta:
        entry = self._entry_protection_stash.get(intent_key)
        if not isinstance(entry, dict):
            return _ProtectionStashDelta(
    ...
    def _restore_protection_preimage(
        self,
        preimage: Mapping[str, Any],
    ) -> None:
        for intent_key, entry in preimage.items():
            key = str(intent_key)
            if entry is False:
                self._entry_protection_stash.pop(
                    key,
                    None,
                )
                continue
    ...
    def _on_durable_io_result(
        ...
        if isinstance(result, _IntentReceiptTransitionResult):
            self._on_intent_receipt_transition_result(result)
            return
        ...
        latest_version = self._protection_continuation_versions.get(
            result.task.continuation_key
        )
        if version != latest_version:
            return
        self._protection_continuation_versions.pop(
            result.task.continuation_key,
            None,
        )
    ...
    def _on_intent_receipt_transition_result(
        self,
        result: _IntentReceiptTransitionResult,
    ) -> None:
        task = result.task
        ...
        if not result.transitioned:
            receipt_status = self._intent_receipt_status(
                task.intent
            )
    ...
    def _queue_exchange_refresh(
        self,
        *,
        target_client_order_ids: Iterable[str] = (),
        success_continuation: Mapping[str, Any] | bool = False,
        failure_continuation: Mapping[str, Any] | bool = False,
    ) -> bool:
        if self._strategy_stopping:
    ...
    def _run_protection_continuation(
        ...
        if kind == "retry_pending_take_profit_disables":
            ...
            return True
        if kind == "retry_exchange_state_dependents":
            self._retry_pending_opening_reconciliations(
                continuation.get("opening_intent_ids", ())
            )
            self._retry_pending_take_profit_disables()
            return True
        if kind == "retry_pending_opening_reconciliations":
            self._retry_pending_opening_reconciliations(
                continuation.get("opening_intent_ids", ())
            )
            return True
        ...
        if kind == "handle_intent_after_exchange_refresh":
            ...
            if (
                not self._exchange_orders_are_fresh()
                and receipt_status
                not in {"DISPATCHED", "RESUBMITTING"}
            ):
                self._handle_intent(
                    intent,
        ...
        if kind == "intent_exchange_refresh_failed":
            ...
            action = str(
                getattr(raw_action, "value", raw_action)
            )
    ...
    def _complete_plan_submission(
        ...
        *,
        ...
        if submitted:
            ...
            if not preserve_resubmitting_receipt:
    ...
    def _continue_entry_submit(
        ...
        if not self._opening_side_effect_allowed(plan):
            ...
            queued = self._queue_entry_protection_stash_persist(
                ...
                changed_intent_keys=continuation.get(
                    "protection_changed_keys",
                    (str(plan.intent_id),),
                ),
            )
        ...
        if self._submit_order_plan(plan):
            return self._complete_plan_submission(
                ...
                source_intent,
                True,
                preserve_resubmitting_receipt=bool(
                    continuation.get(
                ...
                if submitted_plans:
                    self._record_denial(
                        OrderDenied(
                            "zone_partial_submit",
                            (
                                f"submitted={len(submitted_plans)} "
                                f"failed={plan.client_order_id}"
                            ),
                        )
                    )
                    self._defer_opening_reconciliation(
    ...
    def _handle_intent(
        ...
        *,
        ...
        if (
            ...
            and not exchange_refresh_degraded
        ):
            target_client_order_ids: tuple[str, ...] = ()
            if action in {"open_position", "add_position"}:
                target_client_order_ids = (
        ...
        if (
            ...
            reconciled = self._reconcile_existing_opening(
                ...
                require_historical_evidence=(
                    receipt_status
                    in {"DISPATCHED", "RESUBMITTING"}
                ),
            )
            if not self._opening_reconciliation_allows_execution(
                intent,
                receipt_status=receipt_status,
                reconciled=reconciled,
                resubmit_authorized=opening_resubmit_authorized,
            ):
                return
    ...
    def _exchange_orders_are_fresh(self) -> bool:
        mirror = self._exchange_state_mirror
        checker = getattr(
            mirror,
            "orders_are_fresh",
    ...
    def _stash_entry_protection(
        ...
        *,
        ...
        changed_intent_keys: set[str] = set()
        protection_preimage: dict[str, Any] = {}
        for key, other in tuple(self._entry_protection_stash.items()):
            ...
            if other_source_message_id == source_message_id:
                ...
                continue
            protection_preimage[str(key)] = copy.deepcopy(
                other
            )
            changed_intent_keys.add(str(key))
        ...
        if stop_loss is None and not take_profits:
            prepared_continuation = continuation
            if isinstance(continuation, Mapping):
                prepared_continuation = {
                    **dict(continuation),
                    "protection_changed_keys": tuple(
                        sorted(changed_intent_keys)
                    ),
                    "protection_preimage": protection_preimage,
                }
            queued = self._queue_entry_protection_stash_persist(
                continuation=prepared_continuation,
                changed_intent_keys=tuple(
                    sorted(changed_intent_keys)
                ),
            )
            if not queued:
                self._restore_protection_preimage(
                    protection_preimage
                )
            return queued
        ...
        if same_source_owner is not None:
            ...
            if key != str(plan.intent_id):
                protection_preimage[str(key)] = copy.deepcopy(
                    self._entry_protection_stash.get(key)
                )
                changed_intent_keys.add(str(key))
                self._entry_protection_stash.pop(key, None)
                self._cancel_clock_timer(self._PROTECTION_TIMER_PREFIX + key)
        ...
        plan_intent_key = str(plan.intent_id)
        existing_plan_stash = self._entry_protection_stash.get(
            plan_intent_key
        )
        if isinstance(existing_plan_stash, dict):
            protection_preimage[
                plan_intent_key
            ] = copy.deepcopy(existing_plan_stash)
        else:
            protection_preimage[plan_intent_key] = False
        changed_intent_keys.add(plan_intent_key)
        self._entry_protection_stash[plan_intent_key] = {
            "stop_loss": stop_loss,
    ...
    def on_order_accepted(self, event: Any) -> None:
        self._confirm_entry_order_event(event)
    ...
    def on_order_filled(self, event: Any) -> None:
        ...
        if trace.sequence <= 9:
            self._record_intent_id_terminal(
                trace.intent_id,
                "CONFIRMED",
                f"order_filled:{client_order_id}",
            )
    ...
    def _confirm_entry_order_event(self, event: Any) -> None:
        client_order_id = _event_client_order_id(event)
        if client_order_id is None:
            return
        try:
        ...
        if trace.sequence > 9:
            return
        self._clear_opening_reconciliation_id(
            str(trace.intent_id)
        )
        self._record_intent_id_terminal(
            trace.intent_id,
    ...
    def _reconcile_existing_opening(
        self,
        intent: Any,
        *,
        ...
        orders = list(self._cache_orders_all(instrument_id))
        mirror = self._exchange_state_mirror
        ...
        if not callable(orders_for_instrument):
            self._record_denial(
                OrderDenied(
                    "exchange_state_refresh_failed",
                    "exchange order mirror is unavailable",
                )
            )
            return None
        try:
            orders.extend(
                tuple(orders_for_instrument(instrument_id))
            )
        except Exception as exc:
            self._record_opening_reconciliation_denial(
                intent_id,
                OrderDenied(
                    "exchange_state_refresh_failed",
                    repr(exc),
        ...
        for order in orders:
            ...
            try:
                trace = decode_client_order_id(
                    client_order_id
                )
            except ValueError:
                continue
            if (
                str(trace.intent_id) != intent_id
                or trace.sequence > 9
            ):
                continue
    ...
    def _opening_reconciliation_allows_execution(
        self,
        intent: Any,
        *,
        receipt_status: str,
    ...
    def _queue_opening_resubmit_transition(
        ...
        *,
        ...
        if not intent_id:
            return False
        if intent_id in (
            self._opening_resubmit_transition_pending_ids
        ):
            return True
        if self._intent_receipt_transition_handler is None:
            self._halt_durable_io(
                "intent receipt transition handler is unavailable"
            )
            return False
        if (
            not self._durable_io_active
            or self._strategy_stopping
            or self._durable_io_halted_reason
            or self._durable_io_halt_requested_reason
        ):
            self._halt_durable_io(
                "intent receipt transition lane is unavailable"
            )
            return False
        self._opening_resubmit_transition_pending_ids.add(
            intent_id
        )
        accepted = self._durable_io_worker.submit(
    ...
    def _defer_opening_reconciliation(
        ...
        *,
        ...
        current = self._opening_reconciliation_resubmit_allowed.get(
            intent_id,
            True,
        )
    ...
    def _clear_opening_reconciliation(
        ...
        self._clear_opening_reconciliation_id(intent_id)

    def _clear_opening_reconciliation_id(
        self,
        intent_id: str,
    ) -> None:
        self._pending_opening_reconciliations.pop(
            intent_id,
            None,
        )
        self._opening_reconciliation_resubmit_allowed.pop(
            intent_id,
            None,
        )
    ...
    def _record_opening_reconciliation_denial(
        ...
        if key in self._opening_reconciliation_denial_keys:
            return
        self._opening_reconciliation_denial_keys[key] = None
        while len(self._opening_reconciliation_denial_keys) > (
            self._OPENING_RECONCILIATION_DENIAL_LIMIT
        ):
            oldest = next(
                iter(self._opening_reconciliation_denial_keys)
            )
            self._opening_reconciliation_denial_keys.pop(
                oldest,
                None,
            )
        self._record_denial(denial)

    def _next_opening_evidence_intent_batch(
        self,
    ) -> tuple[str, ...]:
        intent_ids = tuple(
            sorted(self._pending_opening_reconciliations)
        )
        if not intent_ids:
            self._opening_evidence_batch_cursor = 0
            return ()
        start = self._opening_evidence_batch_cursor % len(
            intent_ids
        )
        ordered = intent_ids[start:] + intent_ids[:start]
        selected: list[str] = []
        target_count = 0
        for intent_id in ordered:
            intent = self._pending_opening_reconciliations[
                intent_id
            ]
            order_ids = self._expected_opening_client_order_ids(
                intent
            )
            if (
                selected
                and target_count + len(order_ids)
                > self._OPENING_EVIDENCE_TARGET_LIMIT
            ):
                break
            selected.append(intent_id)
            target_count += len(order_ids)
            if target_count >= self._OPENING_EVIDENCE_TARGET_LIMIT:
                break
        self._opening_evidence_batch_cursor = (
            start + len(selected)
        ) % len(intent_ids)
        return tuple(selected)

    def _opening_client_order_ids_for_intents(
        self,
        intent_ids: Iterable[str],
    ) -> tuple[str, ...]:
        client_order_ids: list[str] = []
        for intent_id in intent_ids:
            intent = self._pending_opening_reconciliations.get(
                str(intent_id)
            )
            if intent is None:
                continue
    ...
    def _schedule_opening_reconciliation_retry(self) -> None:
        if (
            ...
            return
        clock = getattr(self, "clock", None)
        set_time_alert = getattr(
            clock,
            "set_time_alert",
            None,
        ) if clock is not None else None
        if not callable(set_time_alert):
            return
        exponent = max(
            self._opening_reconciliation_attempt - 1,
            0,
        )
        delay_seconds = min(2 ** exponent, 45)
        when = self._now() + timedelta(
        ...
        try:
            set_time_alert(
                "intent-opening.reconcile",
                when,
                callback=self._on_opening_reconciliation_alert,
            )
        except TypeError:
            try:
                set_time_alert(
            ...
            return
        self._opening_reconciliation_retry_scheduled = True

    def _on_opening_reconciliation_alert(
        self,
        *_args: Any,
        **_kwargs: Any,
    ) -> None:
        ...
        self._queue_exchange_refresh(
            ...
            success_continuation={
                "kind": "retry_pending_opening_reconciliations",
                "opening_intent_ids": target_intent_ids,
            },
            failure_continuation={
                "kind": "pending_opening_refresh_failed",
    ...
    def on_stop(self) -> None:
        ...
        self._cancel_clock_timer("intent-opening.reconcile")
        self._opening_reconciliation_retry_scheduled = False
    ...
    def on_order_rejected(self, event: Any) -> None:
        self._reject_entry_order_event(event, "order_rejected")
        self._on_protection_order_terminal(event, count_retry=True)

    def on_order_denied(self, event: Any) -> None:
        self._reject_entry_order_event(event, "order_denied")
        self._on_protection_order_terminal(event, count_retry=True)

    def on_order_canceled(self, event: Any) -> None:
        self._reject_entry_order_event(event, "order_canceled")
    ...
    def on_order_expired(self, event: Any) -> None:
        self._reject_entry_order_event(event, "order_expired")
        self._on_protection_order_terminal(event, count_retry=True)

    def _reject_entry_order_event(
        self,
        event: Any,
        event_name: str,
    ) -> None:
    ...
    def _absorb_management_plan(
        ...
        *,
        ...
        if self._queue_entry_protection_stash_persist(
            ...
            changed_intent_keys=tuple(
                intent_key
                for intent_key, _stash in targeted
            ),
        ):
            return True
        for key, value in preimage.items():
    ...
    def _record_denial(self, denial: OrderDenied) -> None:
        ...
        overflow = len(self.denials) - self._DENIAL_HISTORY_LIMIT
        if overflow > 0:
            del self.denials[:overflow]
        log = getattr(self, "log", None)
        if log is not None and hasattr(log, "error"):
    ...
    def _record_intent_terminal(
        ...
        if intent_id is False:
            return False
        return self._record_intent_id_terminal(
            intent_id,
            status,
            detail,
        )

    def _record_intent_id_terminal(
        self,
        intent_id: Any,
        status: str,
        detail: str,
