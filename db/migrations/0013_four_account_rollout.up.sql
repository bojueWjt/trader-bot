    ...
    ADD CONSTRAINT ck_reviewed_release_rollout_events_shape
        CHECK (
            ...
            OR (
                ...
                AND (
                    (
                        from_phase = 'account_a_canary'
                        AND to_phase IN ('account_b_rollout', 'aborted')
                    )
                    OR (
                        from_phase = 'account_b_rollout'
                        AND to_phase IN ('account_c_rollout', 'aborted')
                    )
                    OR (
                        from_phase = 'account_c_rollout'
                        AND to_phase IN ('account_d_rollout', 'aborted')
                    )
                    OR (
                        from_phase = 'account_d_rollout'
                        AND to_phase IN ('fleet_complete', 'aborted')
                    )
                )
            )
        )
...
BEGIN
    ...
    IF NOT (
        ...
        OR (
            ...
            AND NEW.phase IN ('account_c_rollout', 'aborted')
        )
        OR (
            OLD.phase = 'account_c_rollout'
            AND NEW.phase IN ('account_d_rollout', 'aborted')
        )
        OR (
            OLD.phase = 'account_d_rollout'
...
BEGIN
    IF EXISTS (
        ...
        RAISE EXCEPTION
            'release active maintenance fence before four-account migration';
    END IF;
END
$$;
...
ALTER TABLE control_plane_maintenance_fences
    ADD CONSTRAINT ck_control_plane_maintenance_fence_evidence
        CHECK (
            ...
            AND jsonb_array_length(account_evidence) = 4
            AND account_evidence @> '[
...
DECLARE
    ...
    seen_account_c boolean := false;
    seen_account_d boolean := false;
BEGIN
    IF (
    ...
    LOOP
        IF heartbeat_record.account_id = 'account-a' THEN
            IF seen_account_a THEN
                RAISE EXCEPTION
                    'maintenance fence requires one fresh heartbeat for each account-a through account-d';
            END IF;
            seen_account_a := true;
        ELSIF heartbeat_record.account_id = 'account-b' THEN
            ...
            END IF;
            seen_account_b := true;
        ELSIF heartbeat_record.account_id = 'account-c' THEN
            IF seen_account_c THEN
                RAISE EXCEPTION
                    'maintenance fence requires one fresh heartbeat for each account-a through account-d';
            END IF;
            seen_account_c := true;
        ELSIF heartbeat_record.account_id = 'account-d' THEN
            IF seen_account_d THEN
                RAISE EXCEPTION
                    'maintenance fence requires one fresh heartbeat for each account-a through account-d';
            END IF;
            seen_account_d := true;
        ...
        IF (
            ...
            RAISE EXCEPTION
                ...
                heartbeat_record.account_id;
        END IF;
        IF heartbeat_record.node_id IS DISTINCT FROM (
            'nautilus-node-' || heartbeat_record.account_id
        ) THEN
            RAISE EXCEPTION
                '% heartbeat node_id must equal nautilus-node-%',
                heartbeat_record.account_id,
                heartbeat_record.account_id;
        END IF;
        IF (
    ...
    IF (
        NOT seen_account_a
        OR NOT seen_account_b
        OR NOT seen_account_c
        OR NOT seen_account_d
        OR jsonb_array_length(evidence) <> 4
    ) THEN
        RAISE EXCEPTION
            'maintenance fence requires one fresh heartbeat for each account-a through account-d';
    END IF;
...
BEGIN
    ...
    IF current_identity IS DISTINCT FROM expected_identity THEN
        RAISE EXCEPTION
            'maintenance fence A-D node, writer, Redis, or release identity drifted';
    END IF;
    IF EXISTS (
        SELECT 1
