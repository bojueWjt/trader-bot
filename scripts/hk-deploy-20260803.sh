...
API_EXTENSIONS_PACKAGE_FILES=(position_protection.py v1_mirror.py v1_trace.py v1_outcomes.py outcomes_kpis.py)
...
for h in "${CHANGED_HOST[@]:-}"; do
  case "$h" in
    ...
    api_extensions_package)
      ...
      for api_extension_file in "${API_EXTENSIONS_PACKAGE_FILES[@]}"; do
        install_payload_atomically "host/api_extensions/$api_extension_file" \
          "$API_EXTENSIONS_PACKAGE_TGT/$api_extension_file"
      done
      ;;
    audit_package)
