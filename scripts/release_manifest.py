#!/usr/bin/env python3
"""Build and verify the trader-v3 two-node release identity.

The manifest binds a reviewed source bundle to the currently selected image,
the normalized A/B node configuration, and the dependency lock. Live
verification checks both containers without printing environment values or
...
from packages.runtime_resource_contract import (  # noqa: E402
    ABSENT,
    CONTROL_PLANE_SESSION_DEFAULTS as CONTROL_PLANE_SESSION_RESOURCE_DEFAULTS,
    REPORTER_WORKERS_DEFAULTS as REPORTER_WORKER_RESOURCE_DEFAULTS,
    SCHEMA_VERSION as RUNTIME_RESOURCES_SCHEMA_VERSION,
    STRATEGY_DURABLE_IO_DEFAULTS as STRATEGY_DURABLE_IO_RESOURCE_DEFAULTS,
    TERMINAL_EXCHANGE_DEFAULTS as TERMINAL_EXCHANGE_RESOURCE_DEFAULTS,
...
def validate_migration_manifest(
    ...
    *,
    ...
    if runner_path != MIGRATION_RUNNER_PATH:
        raise ReleaseManifestError(
            "migration runner must equal canonical 0010 runner"
        )
    ...
    if up != MIGRATION_UP_PATH:
        raise ReleaseManifestError(
            "migration up path must equal canonical 0010 migration"
        )
    if down != MIGRATION_DOWN_PATH:
        raise ReleaseManifestError(
            "migration down path must equal canonical 0010 migration"
        )
...
def _runtime_identity(
    ...
    if len(set(config_hashes)) != 1:
        raise ReleaseManifestError(
            "A/B normalized config hashes differ"
        )
...
def _runtime_image_identity(
    ...
    if len(image_digests) != 1:
        raise ReleaseManifestError("A/B image content digests differ")
    ...
    *,
    ...
    for result in results[1:]:
        if result["source_hashes"] != reference_hashes:
            raise ReleaseManifestError(
                "A/B mounted source byte hashes differ"
            )
    return results
...
def _parse_node_config_specs(
    ...
    if set(specs) != REQUIRED_ACCOUNTS:
        raise ReleaseManifestError(
            "exactly account-a and account-b --node-config values are required"
        )
    return specs
...
def _parse_containers(
    ...
    *,
    ...
    if len(containers) not in {1, 2} or len(set(containers)) != len(
        containers
    ):
        raise ReleaseManifestError(
            "one or two distinct containers are required"
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    capture_parser = subparsers.add_parser(
        "capture",
        help="capture reviewed bundle plus current A/B image/config identity",
    )
