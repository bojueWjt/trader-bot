#!/usr/bin/env python3
"""Fail-closed single-account SOLUSDT canary round-trip executor."""
...
RELEASE_SOURCE_MANIFEST_SCHEMA = "trader-v3-release-source/v1"
...
class ExecutionTarget:
    ...
    node_id: str
    rollout_phase: str
    permit_store_path: Path
    permit_store_id: str
    release_gate_schema: str
    safety_gate_schema: str
    emergency_close_gate_schema: str
    permit_schema: str
    ledger_schema: str
...
def _execution_target(
    ...
    schema_prefix = f"trader-v3-{account_id}-live"
    return ExecutionTarget(
        ...
        release_gate_schema=f"{schema_prefix}-release-gate/v1",
        safety_gate_schema=f"{schema_prefix}-safety-gate/v1",
        emergency_close_gate_schema=(
            f"{schema_prefix}-emergency-close-gate/v1"
        ),
        permit_schema=f"{schema_prefix}-permit/v1",
        ledger_schema=f"{schema_prefix}-permit-ledger/v1",
    )


SUPPORTED_EXECUTION_TARGETS: Mapping[str, ExecutionTarget] = MappingProxyType({
    "account-a": _execution_target(
        "account-a",
        "nautilus-node-account-a",
        "account_a_canary",
    ),
    "account-b": _execution_target(
        "account-b",
        "nautilus-node-account-b",
        "account_b_rollout",
    ),
    "account-c": _execution_target(
        "account-c",
        "nautilus-node-account-c",
        "account_c_rollout",
    ),
    "account-d": _execution_target(
        "account-d",
        "nautilus-node-account-d",
        "account_d_rollout",
    ),
})
DEFAULT_EXECUTION_TARGET = SUPPORTED_EXECUTION_TARGETS["account-a"]

# Backward-compatible account-a aliases.
ACCOUNT_ID = DEFAULT_EXECUTION_TARGET.account_id
DEFAULT_NODE_ID = DEFAULT_EXECUTION_TARGET.node_id
DEFAULT_ROLLOUT_PHASE = DEFAULT_EXECUTION_TARGET.rollout_phase
DEFAULT_LIVE_PERMIT_LEDGER_PATH = (
    DEFAULT_EXECUTION_TARGET.permit_store_path
)
LIVE_PERMIT_STORE_ID = DEFAULT_EXECUTION_TARGET.permit_store_id
RELEASE_GATE_SCHEMA = DEFAULT_EXECUTION_TARGET.release_gate_schema
SAFETY_GATE_SCHEMA = DEFAULT_EXECUTION_TARGET.safety_gate_schema
EMERGENCY_CLOSE_GATE_SCHEMA = (
    DEFAULT_EXECUTION_TARGET.emergency_close_gate_schema
)
PERMIT_SCHEMA = DEFAULT_EXECUTION_TARGET.permit_schema
LEDGER_SCHEMA = DEFAULT_EXECUTION_TARGET.ledger_schema
...
def resolve_execution_target(
    ...
    node_id: Any = DEFAULT_NODE_ID,
    rollout_phase: Any = DEFAULT_ROLLOUT_PHASE,
) -> ExecutionTarget:
    normalized_account = str(account_id or "").strip()
    target = SUPPORTED_EXECUTION_TARGETS.get(normalized_account)
    if target is None:
        supported = ", ".join(sorted(SUPPORTED_EXECUTION_TARGETS))
        raise LiveTradeExecutionError(
            f"account_id must be one of: {supported}"
        )
    normalized_node = str(node_id or "").strip()
    if normalized_node != target.node_id:
        raise LiveTradeExecutionError(
            f"{target.account_id} node_id must be {target.node_id}"
        )
    normalized_phase = str(rollout_phase or "").strip()
    if normalized_phase != target.rollout_phase:
        raise LiveTradeExecutionError(
            f"{target.account_id} rollout_phase must be "
            f"{target.rollout_phase}"
        )
    return target
...
class PositionSnapshot:
    side: str
    quantity: Decimal
    exchange_confirmed_open_fill_quantity: Decimal
    fetched_at: datetime
    evidence_sha256: str
    ...
    def verify(
        ...
        *,
        ...
        with tempfile.TemporaryDirectory(
            prefix="account-stall-live-signature-"
        ) as temporary_root:
    ...
    def __init__(
        ...
        *,
        ...
        self._temporary_root = tempfile.TemporaryDirectory(
            prefix="account-stall-live-adapter-"
        )
...
class DryRunAdapter:
        ...
        payload.update(
            {
                ...
                "exchange_confirmed_open_fill_quantity": (
                    _decimal_text(self._authorization.quantity)
                ),
    ...
    def submit_close(
        ...
        protected_baseline_quantity = _decimal(
            request.get("protected_baseline_quantity", "0"),
            "protected baseline quantity",
            non_negative=True,
        )
        self._position_quantity = protected_baseline_quantity
...
class SingleUsePermitStore:
    def __init__(
        ...
        *,
        ...
        self._ledger_schema = _required_text(
            ledger_schema,
            "permit ledger schema",
        )
    ...
    def validate_live_binding(
        ...
        target = resolve_execution_target(
            ...
            authorization.rollout_phase,
        )
        if authorization.release.permit_store_id != (
            target.permit_store_id
        ):
            raise LiveTradeExecutionError(
                "signed permit store identity differs from account"
            )
        signed_path = Path(authorization.release.permit_store_path)
        if signed_path != target.permit_store_path:
            raise LiveTradeExecutionError(
                "signed permit ledger path differs from account"
            )
        if self._store_id != authorization.release.permit_store_id:
            raise LiveTradeExecutionError(
                "permit store identity differs from signed release"
            )
        if self._ledger_schema != target.ledger_schema:
            raise LiveTradeExecutionError(
                "permit ledger schema differs from account"
            )
        if self._testing_allow_live_path_override:
            return
        if self._path != target.permit_store_path:
            raise LiveTradeExecutionError(
    ...
    def _load(self) -> dict[str, Any]:
        ...
        if payload.get("schema_version") != self._ledger_schema:
            raise LiveTradeExecutionError(
...
class AccountALiveTradeExecutor:
    def __init__(
        ...
        *,
        ...
        target: ExecutionTarget = DEFAULT_EXECUTION_TARGET,
        mode: str = "dry-run",
    ...
    def execute(
        ...
        _validate_authorization_for_execution(
            authorization,
        ...
        cleanup_result = CleanupResult(
            ...
            protected_baseline_quantity=Decimal(0),
        ...
        try:
            ...
            resume_ack = self._adapter.resume(resume_request)
            resume_hash = _validate_ack(
                resume_ack,
                authorization,
                action="resume",
            ...
            actual_open_notional = observation.actual_open_notional_usdt
            ...
            cleanup_result = self._flatten_target_risk(
                ...
                journal_enabled=journal_enabled,
                open_exchange_not_before=open_dispatched_at,
            ...
            if cleanup_result.final_snapshot is None:
                raise LiveTradeExecutionError(
                    "final safety snapshot is missing"
                )
            final_loss = _decimal(
                cleanup_result.final_snapshot.get(
            ...
            if failure_reason and not cleanup_complete:
                try:
                    emergency_result = self._flatten_target_risk(
                        ...
                        journal_enabled=journal_enabled,
                        open_exchange_not_before=open_dispatched_at,
                        expected_close_quantity=open_filled_quantity,
                    )
            ...
            finished_halted, halt_error = self._halt_with_recovery(
                authorization,
                trail,
                reason=halt_reason,
                journal_enabled=journal_enabled,
        ...
        if finished_halted is not True:
            passed = False
        if cleanup_result.safe is not True:
            passed = False
        if failure_reason:
    ...
    def _wait_for_open_terminal(
        ...
        *,
        ...
        for observation_index in range(1, self._max_observations + 1):
            ...
            raw = self._adapter.observe(observe_request)
            latest = _parse_observation(
                raw,
                authorization,
                now=_utc_now(self._clock),
            )
    ...
    def _flatten_target_risk(
        ...
        *,
        reason: str,
        journal_enabled: bool,
        open_exchange_not_before: datetime | None,
        ...
        protected_baseline_quantity = Decimal(0)
        protected_baseline_side = "FLAT"
        risk_state_confirmed = False
        exchange_not_before = open_exchange_not_before
        ...
        for attempt in range(1, self._recovery_max_attempts + 1):
            ...
            if open_exchange_not_before is not None:
                position_request["open_exchange_not_before"] = (
                    open_exchange_not_before.isoformat()
                )
            ...
            if close_attempted and not close_confirmed:
                ...
                try:
                    ...
                    if filled_quantity != close_quantity:
                        raise LiveTradeExecutionError(
                            "close filled quantity differs from "
                            "exchange-confirmed open fill"
                        )
            ...
            if close_confirmed:
                if (
                    ...
                    break
                last_errors = [
                    "target position differs from the protected baseline "
                    "after the authorized close quantity was filled"
                ]
                break
            ...
            if position.quantity < exchange_confirmed_fill:
                last_errors = [
                    "target position is smaller than exchange-confirmed "
                    "canary open fill"
                ]
                break
            close_quantity = exchange_confirmed_fill
            protected_baseline_quantity = (
                position.quantity - close_quantity
            )
            protected_baseline_side = position.side
            if protected_baseline_quantity == 0:
                protected_baseline_side = "FLAT"
            close_attempted = True
            if protected_baseline_quantity > 0:
                trail.record(
                    "protected_position_baseline_recorded",
                    {
                        "exchange_confirmed_open_fill_quantity": (
                            _decimal_text(close_quantity)
                        ),
            ...
            if open_exchange_not_before is not None:
                close_request["open_exchange_not_before"] = (
                    open_exchange_not_before.isoformat()
                )
            ...
            for attempt in range(
                ...
                try:
                    ...
                    final_snapshot = _validate_final_snapshot(
                        ...
                        expected_position_side=protected_baseline_side,
                    )
                    self._recovery_journal_complete(
                        authorization,
                        trail,
        ...
        if not risk_state_confirmed:
            last_errors.append(
                "recovery could not prove protected target position state"
            )
        ...
        return CleanupResult(
            ...
            protected_baseline_quantity=protected_baseline_quantity,
            protected_baseline_side=protected_baseline_side,
    ...
    def _run_journal_write(
        ...
        worker = threading.Thread(
            target=run,
            name=f"{self._target.account_id}-live-journal-write",
            daemon=True,
        )
        worker.start()
    ...
    def _build_evidence(
        ...
        *,
        ...
        execution_details = {
            ...
            "protected_baseline_quantity": _decimal_text(
                cleanup_result.protected_baseline_quantity
            ),
            "protected_baseline_side": (
                cleanup_result.protected_baseline_side
            ),
...
def load_authorization(
    ...
    *,
    execute_live: bool,
    expected_target: ExecutionTarget = DEFAULT_EXECUTION_TARGET,
    ...
    target = resolve_execution_target(
        expected_target.account_id,
        expected_target.node_id,
        expected_target.rollout_phase,
    )
...
def _validate_authorization_documents(
    ...
    *,
    ...
    _require_schema(
        release_gate,
        target.release_gate_schema,
        "release gate",
    )
    _require_schema(
        safety_gate,
        target.safety_gate_schema,
        "safety gate",
    )
    _require_schema(
        emergency_gate,
        target.emergency_close_gate_schema,
        "emergency close gate",
    )
    _require_schema(permit, target.permit_schema, "permit")
    ...
    release_identity = ReleaseIdentity(
        ...
        release_root_path=str(
            _required_absolute_path(
                release_gate.get("release_root_path"),
                "release gate release_root_path",
            )
        ),
        release_source_manifest_sha256=_required_sha256(
            release_gate.get("release_source_manifest_sha256"),
            "release gate release_source_manifest_sha256",
        ),
    ...
    if release_identity.permit_store_id != target.permit_store_id:
        raise LiveTradeExecutionError(
            "release gate permit store identity mismatch"
        )
    if Path(release_identity.permit_store_path) != (
        target.permit_store_path
    ):
        raise LiveTradeExecutionError(
            "release gate permit ledger path mismatch"
        )
    _require_target_identity(release_gate, target, "release gate")
    _validate_document_window(release_gate, "release gate", now)

    _require_target_identity(safety_gate, target, "safety gate")
    ...
    for field_name, timestamp in health_timestamps.items():
    ...
    if testnet_age < timedelta(0) or testnet_age > timedelta(days=1):
        raise LiveTradeExecutionError(
            "emergency close testnet verification is stale or future"
        )
    ...
    _require_target_identity(permit, target, "permit")
    ...
    close_intent_id = deterministic_close_intent_id(
        intent_id,
...
def deterministic_close_intent_id(
    intent_id: Any,
    *,
    account_id: str = ACCOUNT_ID,
) -> str:
    intent_uuid = UUID(str(intent_id))
    normalized_account_id = _required_text(
        account_id,
        "close intent account_id",
    )
    target = SUPPORTED_EXECUTION_TARGETS.get(normalized_account_id)
    if target is None:
        raise LiveTradeExecutionError(
            "close intent account target is unsupported"
        )
    close_uuid = uuid5(
        intent_uuid,
        f"trader-v3/{target.account_id}/{SYMBOL}/close",
    )
    return str(close_uuid)


def deterministic_close_client_order_id(
    intent_id: Any,
    *,
    account_id: str = ACCOUNT_ID,
) -> str:
...
def _validate_authorization_for_execution(
    ...
    *,
    ...
    if authorization.account_id != target.account_id:
        raise LiveTradeExecutionError(
            f"execution account must be {target.account_id}"
        )
    if authorization.node_id != target.node_id:
        raise LiveTradeExecutionError(
            f"execution node must be {target.node_id}"
        )
    if authorization.rollout_phase != target.rollout_phase:
        raise LiveTradeExecutionError(
            "execution rollout phase must be "
            f"{target.rollout_phase}"
        )
...
def _parse_position(
    ...
    *,
    ...
    exchange_confirmed_open_fill_quantity = _decimal(
        payload.get("exchange_confirmed_open_fill_quantity"),
        "exchange_confirmed_open_fill_quantity",
        non_negative=True,
    )
    if exchange_confirmed_open_fill_quantity > authorization.quantity:
        raise LiveTradeExecutionError(
            "exchange-confirmed open fill exceeds authorization"
        )
...
def _validate_final_snapshot(
    ...
    *,
    now: datetime,
    expected_position_quantity: Decimal = Decimal(0),
    expected_position_side: str = "FLAT",
) -> Mapping[str, Any]:
    _validate_adapter_identity(payload, authorization)
    _require_evidence_source(
        payload,
    ...
    normalized_expected_quantity = _decimal(
        expected_position_quantity,
        "expected final position quantity",
        non_negative=True,
    )
    normalized_expected_side = _required_text(
        expected_position_side,
        "expected final position side",
    ).upper()
    if normalized_expected_quantity == 0:
        if normalized_expected_side != "FLAT":
            raise LiveTradeExecutionError(
                "zero protected baseline requires FLAT side"
            )
    elif normalized_expected_side not in {"LONG", "SHORT"}:
        raise LiveTradeExecutionError(
            "non-zero protected baseline requires LONG or SHORT side"
        )
    if position_quantity != normalized_expected_quantity:
        raise LiveTradeExecutionError(
            "final target position quantity differs from protected baseline"
        )
    position_side = _required_text(
        payload.get("position_side", "FLAT"),
        "final position_side",
    ).upper()
    if position_side != normalized_expected_side:
        raise LiveTradeExecutionError(
            "final target position side differs from protected baseline"
        )
    expected_flat = normalized_expected_quantity == 0
    if payload.get("target_symbol_flat") is not expected_flat:
        raise LiveTradeExecutionError(
            "final target_symbol_flat differs from protected baseline"
        )
    ...
    normalized["position_quantity"] = _decimal_text(position_quantity)
    normalized["position_side"] = position_side
    normalized["protected_baseline_quantity"] = _decimal_text(
        normalized_expected_quantity
    )
    normalized["protected_baseline_side"] = normalized_expected_side
...
def _validate_ack(
    ...
    *,
    ...
    if (
        expected_intent_id
        and payload.get("intent_id") != expected_intent_id
    ):
        raise LiveTradeExecutionError(
            f"adapter intent ID mismatch: {action}"
        )
...
def _validate_adapter_identity(
    ...
    *,
    ...
    expected = _identity_payload(authorization)
    if expected_intent_id:
        expected["intent_id"] = expected_intent_id
    for field_name, expected_value in expected.items():
...
def _require_target_identity(
    ...
    if payload.get("account_id") != target.account_id:
        raise LiveTradeExecutionError(
            f"{label} account must be {target.account_id}"
        )
    if payload.get("node_id") != target.node_id:
        raise LiveTradeExecutionError(
            f"{label} node must be {target.node_id}"
        )
    if payload.get("rollout_phase") != target.rollout_phase:
        raise LiveTradeExecutionError(
            f"{label} rollout phase must be {target.rollout_phase}"
        )
...
def validate_live_adapter(
    ...
    *,
    ...
    release_root = _required_absolute_path(
        expected_release_root,
        "signed release root path",
    )
    try:
        resolved_release_root = release_root.resolve(strict=True)
    except OSError as exc:
        raise LiveTradeExecutionError(
            f"cannot resolve signed release root: {release_root}"
        ) from exc
    if resolved_release_root != release_root:
        raise LiveTradeExecutionError(
            "signed release root must be canonical and contain no symlinks"
        )
    if not release_root.is_dir():
        raise LiveTradeExecutionError(
            "signed release root must be a directory"
        )
    expected_target = release_root / LIVE_ADAPTER_RELEASE_PATH
    if target != expected_target:
        raise LiveTradeExecutionError(
            "live adapter path differs from signed release root"
        )
    ...
    source_manifest_path = release_root / RELEASE_SOURCE_MANIFEST_NAME
    source_manifest_bytes = _read_protected_file(
    ...
    normalized_manifest_hash = _required_sha256(
        ...
        "signed release source manifest sha256",
    )
    if _sha256_bytes(source_manifest_bytes) != normalized_manifest_hash:
        raise LiveTradeExecutionError(
            "release source manifest hash differs from signed release"
        )
    ...
    if (
        ...
        != RELEASE_SOURCE_MANIFEST_SCHEMA
    ):
        raise LiveTradeExecutionError(
            "release source manifest schema mismatch"
        )
    raw_files = source_manifest.get("files")
    if not isinstance(raw_files, list):
        raise LiveTradeExecutionError(
            "release source manifest files must be a list"
        )
    adapter_entries = [
        item
    ...
    adapter_entry = adapter_entries[0]
    if adapter_entry.get("source_path") != LIVE_ADAPTER_SOURCE_PATH:
        raise LiveTradeExecutionError(
            "release source manifest live adapter source mismatch"
        )
    manifest_adapter_hash = _required_sha256(
        adapter_entry.get("sha256"),
        "release source manifest live adapter sha256",
    )
    if manifest_adapter_hash != normalized_hash:
        raise LiveTradeExecutionError(
            "signed live adapter hash differs from release source manifest"
        )
    if adapter_entry.get("source_git_mode") != "100755":
        raise LiveTradeExecutionError(
            "release source manifest live adapter mode mismatch"
        )
    manifest_size = adapter_entry.get("size")
    if (
        isinstance(manifest_size, bool)
        or not isinstance(manifest_size, int)
        or manifest_size != len(payload)
    ):
        raise LiveTradeExecutionError(
            "release source manifest live adapter size mismatch"
        )
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Execute one fail-closed single-account SOLUSDT round trip."
        )
    )
    parser.add_argument(
        "--account-id",
        default=ACCOUNT_ID,
        choices=tuple(SUPPORTED_EXECUTION_TARGETS),
    )
    parser.add_argument("--node-id", default=DEFAULT_NODE_ID)
    parser.add_argument(
        "--rollout-phase",
        default=DEFAULT_ROLLOUT_PHASE,
    )
...
def _execute_live_from_args(
    ...
    if args.permit_ledger is not None:
        ...
        if requested_ledger != target.permit_store_path:
            raise LiveTradeExecutionError(
                "live permit ledger path is fixed"
            )
    ...
    with operation_lock:
        with TerminationSignalGuard():
            ...
            validated_adapter = validate_live_adapter(
                ...
                expected_release_root=Path(
                    authorization.release.release_root_path
                ),
                expected_source_manifest_sha256=(
                    authorization.release
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if execute_live:
            result = _execute_live_from_args(args, paths, target)
        else:
            result = _execute_dry_run_from_args(args, paths, target)
