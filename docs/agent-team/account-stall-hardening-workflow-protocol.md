...
1. 不可变节点镜像、release manifest、A-D image digest 一致性门。
...
6. 故障注入、真实 Redis/Nautilus 重启恢复、四账号受限真实小额交易闭环。
...
- 记录当前分支、HEAD、工作区已有改动、线上 image/mount/file hash、A-D version、Redis 容量和节点状态。
- 建立证据目录，所有读操作输出脱敏。
...
- account-a 到 account-d 一次部署同一 digest，全部保持 HALTED，完成启动对账和只读核验。
- canary 按 account-a、account-b、account-c、account-d 串行推进；前一账号签名闭环后才放行下一账号。
...
- 每个账号完成真实挂单撤单和一个微型 market round trip。
...
- Evidence Auditor 已核对 A-D、manifest、配置和数据库迁移。
- 回滚命令、备份和恢复验证均已准备。
...
- A-D digest、config hash 或 schema version 不一致。
...
- A-D 运行同一 digest 和 config schema。
- 故障注入报告 PASS。
- 四账号真实小额交易分别完成开仓、投影、reduce-only 平仓和账户归零。
