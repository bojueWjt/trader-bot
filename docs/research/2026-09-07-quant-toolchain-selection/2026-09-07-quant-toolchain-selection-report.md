...
| 问题 | 推荐方案 | 总分 | 证据强度 |
|---|---|---|---|
| Q1 Telegram 全量历史 | **Telegram Desktop 官方导出**作规范全量样本；**Telethon 1.44**（次号、独立 session、锁 Codeberg tag）只做按 id 补拉与 edit_date/增量对照；tdl 补媒体 | 80 | 高 |
| Q2 抽取与确认 | **LLM Batch + 确定性解析器**去相关组合，分歧交第二家 LLM 裁决，一致集人工抽审 10%；**Label Studio 1.23.0** 社区版确认；锁箱 = JSONL + sha256 manifest + OpenTimestamps | 82 | 高 |
...
| Q4 回测引擎 | **自研差分模拟器为主数字来源**（mark price 触发、资金费率结算、同 bar 乐观/悲观双界）+ **NautilusTrader 1.227.0 作审计引擎**（单 engine 长跑、壳策略调度、按 order tag 归因），两者成交事件序列 diff 即审计产物 | 78 | 高（源码） |
| Q5 特征与统计 | **TA-Lib 0.7.1** + **"Alpha158 变体"**（窗口按墙钟时间、`$vwap` = bar VWAP、维数 ≤ 已闭合正事件数/10）；主 CV = 自写 AFML `PurgedKFold(t1)` + embargo 走前，purgedcv/skfolio 旁证；`arch` 8.0 + 自写事件簇 bootstrap；`imodels` 3.0 规则模型 | 76–85 | 高/中 |
...
2. **回测引擎的正确姿势是"自研为主、Nautilus 审计"，不是反过来。** 本机读 1.227.0 源码证实：模拟交易所不消费 MarkPriceUpdate（`TriggerType::MarkPrice` 落 bid/ask 分支）、回测不结算 funding、bar 驱动时 bid=ask=last。永续合约的三个决定性要素全在 Nautilus 之外，主数字只能来自自研层；Nautilus 的价值是订单状态机与 filter 一致性审计。
3. **样本管道的最大风险在账号与相关错误，不在工具。** 2025-05 起 Telegram 冻结机制 = 账号连同 session 作废；两个 LLM 对同一模糊信号约 60% 同向出错（ICML 2025），"两家一致"不是正确性证书。
...
- 已闭合正事件数 N 尚未从全量历史数出，特征维数上限（N/10）待 M0 后确定。
...
|---|---|---|---|---|---|---|---|
| **TDesktop 官方导出（规范全量）** | 70 | 90 | 60 | 70 | 90 | 76 | 高 |
...
风险与反驳：只读频道历史属低风险类，2025–2026 无"纯只读拉历史被封"公开案例（搜索词 `telegram userbot ban scraping channel history telethon 2025`）；风险集中在新号/虚拟号（Issue #3955/#4150）。审稿人指出全量抓取中一个未知 constructor 会抛 `TypeNotFoundError` 断游标，需外套跳过并记录。

下一步：TDesktop 导出五个频道全量 JSON（含媒体）作规范样本；Telethon 次号对同一频道拉一次带日志（`wait_time=1`、`flood_sleep_threshold=120`）记 FloodWait 基线，比对消息数与 id 集合。止损：账号出现冻结提示即停，回落 TDesktop。

### Q2 LLM 结构化抽取 + 人工确认

| 候选（确认工具） | 能力 | 成熟 | TCO | 对接 | 退出 | 总分 | 置信 |
|---|---|---|---|---|---|---|---|
| **Label Studio 1.23.0 社区版** | 85 | 85 | 80 | 70 | 85 | 81.8 | 高 |
...
- 相关错误：350 个模型三数据集，两模型都错时约 60% 给出同一错答案，越强越相关。[Correlated Errors in LLMs, ICML 2025, https://arxiv.org/pdf/2506.07962]（高）
- OpenTimestamps 客户端 0.7.2（2024-12-31），停更但协议稳定。[PyPI, https://pypi.org/project/opentimestamps-client/]（高）

风险与反驳：截图价格标签读数（OCR + 空间关联）无任何公开基准，已有评测只说明 VLM 看形态不可靠。[gist 40 条信号审计, https://gist.github.com/roman-rr/c1cd675f7c35b68ae5ac281c30080166]（中）。约束库（Instructor/BAML/LangExtract/Outlines）在厂商原生结构化输出下非必需。

下一步：50–100 张带真值截图小基准，schema 每个价格字段带 `source: text|image` + `confidence`；确定性解析器（正则/语法）与 LLM 独立抽取，一致集抽审 10% 报错误率；Label Studio 跑 3 档/5 档止盈样例验证 `<Repeater>` 与导出 diff。止损：图片字段纠错率 > 30% 则全部转人工录入。
...
风险与反驳（对抗轮，全部采纳为闸门）：月包曾静默缺整整两天。[binance-public-data #297, https://github.com/binance/binance-public-data/issues/297]（高）；SPOT 2025-01-01 起时间戳微秒、期货仍毫秒。[README, https://github.com/binance/binance-public-data/blob/master/README.md]（高）；旧 CSV 无表头新有表头混用；DuckDB 磁盘不足可损坏库文件 [#9667]、v1.4 曾破坏兼容。→ 入湖闸门：逐文件 `.CHECKSUM` 校验、按月 bar 数核对、缺口日包回填、时间戳统一毫秒、表头检测；**Parquet 为唯一真身，查询走 `read_parquet` 视图，.duckdb 可随时删除重建，版本进 lockfile。**
...
| 候选 | 能力 | 成熟 | TCO | 对接 | 退出 | 总分 | 置信 |
|---|---|---|---|---|---|---|---|
| **自研差分模拟器（主）+ Nautilus 1.227.0 审计引擎** | 80 | 85 | 65 | 95 | 60 | 77.8 | 高（源码） |
...
风险与反驳：官方 Issue #2194（2025-01）承认 FillModel "only very basic"，#1476 报过 EOD bar 权益曲线算错（高）。审稿人据此主张主次对调，已采纳：主数字来自自研层（必须有单测与对拍用例），Nautilus 审计订单状态机 / filter / OrderList / GTD 一致性；批量按"单 engine 一次长跑、壳策略按时间表调度全部计划、按 order tag 归因"，避免每计划冷启动。LGPL-3.0 对内部不分发使用零义务，不列风险。本机 Python 3.14 + uv 可零编译安装 1.227.0（PyPI 最新 1.231.0）。

下一步：取 20 个唯一可归属纯机器人 episode（M3-E），两引擎分别回放并与 `execution_events` 对账（数量 ≤1 步进、价格 ≤1 tick）；用 markPriceKlines 作驱动 bar 再跑一遍近似 mark 触发。止损：两引擎差异无法解释即停止政策研究，先修模拟器。
...
| 维数上限 | ≤ 已闭合正事件数 N / 10（EPV≥10） | — | 高 | Peduzzi 1996 / van Smeden 2016；舒琴 141 条 ≈ 14 维 |
...
| 主 CV | 自写 AFML `PurgedKFold(t1)` + embargo 走前；`TimeSeriesSplit(gap=)` 零依赖基线；purgedcv 0.1.6 / skfolio 旁证；N<300 不用 CPCV | 74 | 中 | purgedcv 事件级 purge 语义正确但 31 stars 无第三方对拍；skfolio 按行数 purge |
...
| # | PoC | 成功判据 | 止损判据 | 工时 |
|---|---|---|---|---|
| 1 | TDesktop 导出五频道全量 JSON；Telethon 次号同频道对照拉取带日志 | 消息 id 集合一致率 ≥ 99%；FloodWait 基线记录 | 冻结提示 / FloodWait > 1h → 停 | 1 天 |
| 2 | Binance Vision 预热：44 品种 × klines/markPriceKlines/fundingRate/metrics × 2025-01 起，含 CHECKSUM/bar 数/时间戳/表头闸门 | 覆盖矩阵无未解释缺口 | — | 1 天 |
| 3 | 截图价格 OCR 小基准 50–100 张 + 解析器/LLM 双抽取 + 一致集 10% 抽审 | 图片字段纠错率 ≤ 30% | > 30% 转人工录入 | 2 天 + 人工 2 小时 |
| 4 | Label Studio 3/5 档止盈样例（Repeater）+ 导出 diff | 表单可表达、导出含 predictions 与 annotations | 表达不了 → Streamlit 备胎 | 0.5 天 |
| 5 | 自研模拟器 v0（mark 触发 / funding / 双界）+ Nautilus 单 engine 壳策略；20 个纯机器人 episode 双引擎对账 | 数量 ≤1 步进、价格 ≤1 tick；差异全部可解释 | 差异不可解释 → 先修模拟器 | 5 天 |
| 6 | Alpha158 变体在 polars 复现 + 8 算子自写 + 两类前视测试 + 数出已闭合正事件 N | 截断重算一致；N 与维数上限确定 | — | 2 天 |
| 7 | `PurgedKFold(t1)` 自写 + purgedcv 对拍 + 事件簇 bootstrap | 对拍数值一致 | — | 1 天 |
