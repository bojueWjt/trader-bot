...
| Live canary exchange risk | `exchange_state_mirror`、目标节点 `REFRESH_EVIDENCE` ACK、heartbeat 仓位/普通挂单/algo 挂单时间戳 | 5s | recorder 每 3s 自动采集 A-D；每个 canary 在 permit 绑定且 RESUME 完成后、OPEN 前刷新目标节点并等待 ACK | 13h |
