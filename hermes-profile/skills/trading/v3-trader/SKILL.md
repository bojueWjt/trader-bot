...
**用户当前明确指令优先于频道策略、原入场归属和机器人维护规则。** 用户要求平仓、减仓或调整保护时，使用 `--authorized-by-type user`，直接按用户指定账户、方向和数量执行；无需查找原入场 ref、征求原频道许可或再次要求用户授权。用户身份由交互凭证确定，消息标识默认使用本次 `--ref`，无需重复填写 `--authorized-by-id` / `--source-message-id`。原频道、旧 parent/ref 不决定这次用户操作的权限。自动跟单必须继续声明 channel，不能借用 user 身份。交易所实际仓位、明确的操作范围和同 ref 幂等仍约束最终下单；接口受理不等于成交。
...
5.2 **正文点位优先（用户固定约定）**：正文与附图的入场、加仓、止损或止盈点位不一致时，按正文明确给出的点位执行。附图仅补充正文未提供的参数，例如正文写“止盈见图”时读取图中 TP。即使两者相差 10 倍、100 倍，也不得依据附图擅自移动正文小数点、缩放或替换正文点位，不得仅因图文冲突跳过信号或反复要求确认。`--reason` 和结果反馈简要注明“图文点位冲突，按正文执行”。正文自身缺失或矛盾的参数仍按原有缺参规则处理；既有定量、价格合法性及交易所校验照常执行，真实拒因如实反馈。
...
6. **百分比减仓规则**:已有仓位更新信号里出现“减仓/平仓/止盈/锁定/落袋 + X%”时,默认解释为“按当前授权可管理仓减掉 X%”,必须用 `partial --percent X`。CLI 只把百分比转成结构化 `fraction`（`X/100`，`0<fraction<=1`）提交，**禁止**用 `positions_projection`、陈旧镜像、reason 文本或心算改成绝对 `--quantity`。节点在现有授权 scope 内计算：user 用新鲜同向 venue；channel 用 `min(robot_owned, 新鲜同向 venue)`（robot_owned 是全部机器人 stash，不是 per-channel 隔离）。绝对 `--quantity` 原样提交，CLI 不得静默按 step 改写。**禁止把 20%/50% 写成 `close`**：`close` 永远全平；缺比例、0%、>100%、与 `--quantity` 同时出现、或解析失败必须停手上报，不得改走 `close` 当默认。尤其用户已明确约定：`锁定10%利润` 按“减仓当前仓位 10%”处理,不能因为没有给 base quantity 而跳过。只有文本明确写“锁定利润到 X% / 止损锁 X%收益 / 保本+X%”这类不是仓位比例的表达时,才不要减仓,改为移动止损或转人工复核。
...
12. **自动频道管理才校验归属**：处理频道消息时只代表该频道。close/partial/set-sl/set-tps 前查询原入场归属；非本频道不操作。同币种同方向混合多个频道时，自动跟单仅管理本频道可归属数量，不能整仓 close。用户明确操作不受频道归属限制，归属信息可展示但不得阻塞执行。
13. **管理参数**：close/partial/set-sl/set-tps/cancel 使用本次操作的稳定 `--ref`，超时重试复用。自动频道操作另带 `--channel` 和 `--entry-ref`；查不到归属则不自动操作。用户管理只需指定账户、方向、操作参数、reason、ref 和 `--authorized-by-type user`，不要求原入场记录。额外无法对账的手动量与 `aos_*` 不得被自动跟单或保护维护纳入、撤销。
14. **开仓信号时效(2026-09-05 漏单事故规则)**:开仓类信号若消息发布时间距当前超过 30 分钟，只汇报不执行（回复标注 ⏰过期）。仓位管理类（移损/止盈/平仓）不受此限，但必须核对当前仓位；仅自动频道操作另核对归属。无法确认发布时间时，开仓类一律只汇报不执行。
15. **双入场一次提交（2026-09-11 用户修正）**：同一信号两个明确入场位，使用一次 `open`，首腿沿用 `--entry-type market|limit`（limit 带 `--price`），第二腿用 `--second-price`。两腿按同等名义金额、共享一份总风险预算，由控制面定量。使用原信号 ref，不追加 -e1/-e2；返回失败时报告真实原因，禁止回退单腿或换 ref 重试。多于两腿不自动截断，报告暂不支持。已有仓/已提交历史 e1 的信号不得自动补第二腿。
...
# 两个明确入场位：一个 ref，一次提交。两腿同等金额，共用总风险预算。
python3 $V3 open ICPUSDT long --entry-type market --second-price 2.662 --sl 2.58 \
  --reason "Titan双入场" --account account-b \
  --channel -1002198013097 --authorized-by-type channel \
