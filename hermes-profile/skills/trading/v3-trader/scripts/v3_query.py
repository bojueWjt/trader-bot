...
def cmd_positions(args) -> None:
    ...
    for row in _mirror():
        ...
        for p in payload.get("positions") or []:
            ...
            # protections are keyed by symbol only; in hedge mode split the
            # SL/TP orders by exit side (long exits SELL, short exits BUY)
            prot = next((x for x in protections if x.get("symbol") == sym), {})
